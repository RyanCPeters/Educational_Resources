...
denoted by Î» (lambda), is the string containing no symbols. The set of all words over V is denoted
...
The rules that specify when we can replace a string from V\âˆ—, *productions*, such as S â†’ NP VP
</font>
...
G = (V, T, S, P), where V = {a, b, A, B, S}, T = {a, b}, S is the start symbol, and P =
{S â†’ ABa, A â†’ BB, B â†’ ab, AB â†’ b}.

What words are valid?

ba (from ABa, AB â†’ b)  
ababa (from ABa, A â†’ BB, BBA, ababa)  
abababa ?  
abbabbaba ?  
...
For G with the grammar with vocabulary V = {S, A, a, b}, set of terminals T = {a, b}, starting
symbol S, and productions P = {S â†’ aA, S â†’ b, A â†’ aa}. What is L(G), the language of this grammar?
...
Let G be the grammar with vocabulary V = {S, 0, 1}, set of terminals T = {0, 1}, starting symbol
S, and productions P = {S â†’ 11S, S â†’ 0}. What is L(G), the language of this grammar?
...
Give a phrase-structure grammar that generates the set {0^n^1^n^ | n = 0, 1, 2, . . . }.
...
Backusâ€“Naur form (BNF), after John Backus, who invented it, and Peter Naur, who refined it
...
Exercise: FSM state table to diagram
===========================

![](../images/rosen13-example3-table.png)


Exercise: FSM diagram to state table
===========================
...
Exercise: FSM diagram to signal error
===========================
...
Finite State Automota (FSM with no output)
===========================
